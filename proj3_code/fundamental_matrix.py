...
def signed_point_line_errors(x_0s, F, x_1s):
    ...
    for i in range(num_points):
        second_to_first_line = np.dot(F, x_1s[i])
        first_to_second_line = np.dot(F_T, x_0s[i])
        errors.append(point_line_distance(second_to_first_line, x_0s[i]))
        errors.append(point_line_distance(first_to_second_line, x_1s[i]))
    ##############################
